const invoice = (function () {
    ...
    let calcTotal = (from) => {
        ...
        if (table.rows.length > 0) {
            $.each(table.rows, function (i, v) {
                ...
                var discountPercentInline = calcDiscount($(this).find(".itemName").data("item-code"), quantity);
                if (discountPercentInline <= 0) {
                    ...
                    discount = discount = rate * discoutPercent / 100 * quantity;
    ...
    let updateFlatDiscount = (calcTotalFlag) => {
        ...
        $.each(table.rows, function (i, v) {
            var amount = parseFloat($(this).find(".Rate").val());
            if (type === "percent")
                amount = parseFloat($(this).find(".GrossAmount").val());
