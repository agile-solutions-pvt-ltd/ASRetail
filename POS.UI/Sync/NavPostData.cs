    ...
    public class NavPostData
    {
        ...
        public bool PostSalesInvoice(NavSalesInvoice invoice)
        {
            ...
            if (config.StopInvoicePosting == false)
            {
                ...
                {
                    string errorMessage = SyncStatusMessage("Error", "Invoice", invoice.number, response.Content);
                    WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber()  + ".log", errorMessage);
        ...
        public bool PostSalesInvoice(Store store)
        {
            ...
            //var unSyncInvoice = _context.SalesInvoice.Where(x => x.IsNavPosted == false).OrderBy(x => x.Trans_Date_Ad);
            var unSyncInvoice = _context.SalesInvoice.Where(x => x.Chalan_Number == "TempSync" && x.IsNavPosted == false).ToList();
            NavIntegrationService services = _context.NavIntegrationService.FirstOrDefault(x => x.IntegrationType == "SalesInvoicePost");
            ...
            foreach (var salesInvoice in unSyncInvoice)
            {
                try
                {
                    ...
                    if (response.StatusCode == HttpStatusCode.Created || response.StatusCode == HttpStatusCode.OK || response.Content.Contains("already exists"))
                    {
                        ...
                        {
                            string url1 = config.NavApiBaseUrl + "/" + config.NavPath + $"/companies({config.NavCompanyId})/{services.ServiceName}";
                            url1 = url1 + "?$filter=number eq '" + invoice.number + "'";
                            var client1 = NAV.NAVClient(url1, config);
                            var request1 = new RestRequest(Method.GET);
                            IRestResponse<SyncModel<NavSalesInvoice>> response1 = client1.Execute<SyncModel<NavSalesInvoice>>(request1);
                            data = response1.Data.value.FirstOrDefault();
                        }
            ...
            config.Environment += " Finished :: Success " + successCount + " - Error " + errorCount;
            ConfigJSON.Write(config);
            return true;
        }
        [AutomaticRetry(Attempts = 0)]
            ...
            foreach (var i in invoiceBill)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            string errorMessage = SyncStatusMessage("Error", "Invoice Payment", invoiceNumber, response.Content);
                            WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
        ...
        public bool PostSalesInvoiceItem(string invoiceNumber, string invoiceId, string transType)
        {
            ...
            foreach (var item in items)
            {
                ...
                try
                {
                    ...
                    {
                        string errorMessage = SyncStatusMessage("Error", "Invoice Item", items.FirstOrDefault().Invoice_Number.ToString(), response.Content);
                        WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
        ...
        public void PostSalesInvoiceCompletedSignalToNav(Config config, Guid invoiceId, string invoiceNumber)
        {
            ...
            {
                if (response.Content.Contains("Sorry, we just updated this page. Reopen it") || response.Content.Contains("locked"))
                {
                    string errorMessage = SyncStatusMessage("Error", "Invoice Post", invoiceNumber, response.Content);
                    WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
        ...
        public bool PostCreditNote(NavCreditMemo creditNote)
        {
            ...
            {
                string errorMessage = SyncStatusMessage("Error", "Credit Note", creditNote.number, response.Content);
                WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
            ...
            foreach (var credit in unSyncInvoice)
            {
                try
                {
                    ...
                    NavCreditMemo creditNote = new NavCreditMemo()
                    {
                        ...
                        posbillamt = credit.Total_Net_Amount.Value
        ...
        public bool PostCreditNoteItem(string invoiceNumber, string invoiceId)
        {
            ...
            foreach (var item in allUnSyncItemList)
            {
                try
                {
                    ...
                    {
                        string errorMessage = SyncStatusMessage("Error", "Credit Note Item", invoiceId, response.Content);
                        WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
        ...
        public bool UpdateIsNavSyncStatusCreditMemo(IEnumerable<CreditNote> invoice)
        {
            try
            {
                ...
                //update invoice items
                var unSyncInvoiceNumber = invoice.Select(x => x.Credit_Note_Number).ToList();
                var unSyncInvoiceItem = _context.CreditNoteItem.Where(x => unSyncInvoiceNumber.Contains(x.Credit_Note_Number));
                foreach (var item in unSyncInvoiceItem)
                {
                    item.IsNavSync = false;
                    _context.Entry(item).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
        ...
        public void PostCreditInvoiceCompletedSignalToNav(Config config, Guid invoiceId, string invoiceNumber)
        {
            ...
            else
            {
                if (response.Content.Contains("Sorry, we just updated this page. Reopen it"))
                    PostCreditInvoiceCompletedSignalToNav(config, invoiceId, invoiceNumber);
                else
                {
                    string errorMessage = SyncStatusMessage("Error", "Credit Note Post", invoiceNumber, response.Content);
                    WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
            ...
            if (!config.StopCustomerPosting)
            {
                ...
                foreach (var cus in navCustomers)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string errorMessage = SyncStatusMessage("Error", "Customer", cus.number, response.Content);
                            WriteToFile(Path.GetFullPath("logs/"), "NAVSyncLog_" + DateTime.Now.ToString("yyyy_MM") + "_" + Date.GetWeekNumber() + ".log", errorMessage);
        ...
        public string SyncStatusMessage(string statusType, string billType, string id, string message)
        {
            string result = "[" + DateTime.Now.ToString() + "] [" + statusType + "] [" + billType + ":: Number= " + id + "] Message= " + message;
            return result;
        }
    }
}
